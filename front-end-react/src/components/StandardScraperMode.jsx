...
// Helper to generate unique IDs (if not already in a shared util)
const generateUniqueId = () => Date.now().toString(36) + Math.random().toString(36).substring(2, 9);

// SortableFieldRow wrapper component (assuming FieldRow is also imported or defined)
...
export default function StandardScraperMode() {
  const { 
    formData, 
    setFormData, 
    scrapeOperation, 
  ...
  const handleSubmit = async (e) => {
    ...
    startScrapeOperation('standard-dynamic', operationKey); // New type for clarity
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/scrape-dynamic`, { // Assuming this endpoint is still used
        method: "POST",
    ...
    <>
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="space-y-3 pt-4">
          ...
          >
            {isLoadingScrape ? "Scraping..." : "Scrape Standard URL"}
          </Button>
          {isLoadingScrape && (
            <Button
              type="button"
